...
/obj/spacepod
	...
	icon = 'waspstation/goon/icons/obj/spacepods/construction_2x2.dmi'
	icon_state = "pod_1"
	density = 1
	opacity = 0
...
/obj/spacepod/update_icon()
	...
	if(construction_state != SPACEPOD_ARMOR_WELDED)
		...
		if(pod_armor && construction_state >= SPACEPOD_ARMOR_LOOSE)
			var/mutable_appearance/masked_armor = mutable_appearance(icon = 'waspstation/goon/icons/obj/spacepods/construction_2x2.dmi', icon_state = "armor_mask")
	...
	if(pod_armor)
		...
		icon = 'waspstation/goon/icons/obj/spacepods/2x2.dmi'
		icon_state = initial(icon_state)

	if(obj_integrity <= max_integrity / 2)
		add_overlay(image(icon='waspstation/goon/icons/obj/spacepods/2x2.dmi', icon_state="pod_damage"))
		if(obj_integrity <= max_integrity / 4)
			add_overlay(image(icon='waspstation/goon/icons/obj/spacepods/2x2.dmi', icon_state="pod_fire"))
	...
	for(var/cdir in GLOB.cardinals)
		...
		if(left_thrust)
			add_overlay(image(icon = 'waspstation/icons/obj/spacepods/2x2.dmi', icon_state = "rcs_left", dir = cdir))
		if(right_thrust)
			add_overlay(image(icon = 'waspstation/icons/obj/spacepods/2x2.dmi', icon_state = "rcs_right", dir = cdir))
	if(back_thrust)
		var/image/I = image(icon = 'waspstation/icons/obj/spacepods/2x2.dmi', icon_state = "thrust")
