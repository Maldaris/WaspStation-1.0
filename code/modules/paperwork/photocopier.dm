 ...
 */
/obj/machinery/photocopier/proc/do_copy_loop(datum/callback/copy_cb, mob/user, ignore_copies)
	busy = TRUE
	var/copies = ignore_copies ? 1 : num_copies
	var/i
 ...
 */
/obj/machinery/photocopier/proc/make_photo_copy(obj/item/photo/to_copy)
	if(!photo_copy && !to_copy)
		return
	to_copy = to_copy ? to_copy : photo_copy
	var/obj/item/photo/copied_pic = new(loc, to_copy.picture.Copy(color_mode == PHOTO_GREYSCALE ? TRUE : FALSE))
 ...
 */
/obj/machinery/photocopier/proc/make_document_copy(obj/item/documents/to_copy)
	if(!document_copy && !to_copy)
		return
	to_copy = to_copy ? to_copy : document_copy
	var/obj/item/documents/photocopy/copied_doc = new(loc, to_copy)
...
/obj/machinery/photocopier/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/paper))
		...
		to_chat(user, "<span class='warning'>The Blueprint is too large to put into [src]. You need to find something else to record the document.</span>")
	else
		return ..()
